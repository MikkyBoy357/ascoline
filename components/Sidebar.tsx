...
const Sidebar: React.FC<Props> = ({ items, onShowPopup }) => {
          ...
          return permissions.every((p) => {
            return (
              user.permissions.find(
                (up) => p.name === up.name && p.action === up.action,
              ) !== undefined
            );
          });
      } else {
  ...
  if (status === "loading") {
    return <div className="h-screen w-60 bg-white"></div>;
  }
  ...
  return (
    <>
      ...
      <aside
        className={`h-full w-80 sticky overflow-x-auto bg-white border-r border-gray-200 transition-transform duration-300 ease-in-out transform ${
          ...
          <ul className="py-4">
            {items.map((item, index) => (
              <Fragment key={index}>
                <li
                  className={clsx(
                    !checkPermissions(item.permissions) && "hidden",
                    item === item && "font-semibold",
                    isActive(item.url)
                      ? "text-white bg-[#4763e4]"
                      : "text-gray-500",
                    `mb-6 mx-8 py-2 px-4 group rounded-md hover:bg-[#4763e4] hover:text-white transition duration-300 hover:cursor-pointer`,
                  )}
                  onClick={() => {
            ...
            <button
              className={`mb-6 mx-8 py-2 px-4 group rounded-md hover:bg-[#4763e4] text-gray-500 hover:text-white transition duration-300`}
              onClick={() => {
