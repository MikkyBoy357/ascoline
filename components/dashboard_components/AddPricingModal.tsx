...
export const AddPricingModal: React.FC<AddPricingModalProps> = ({
    ...
    if (isModify) {
      ...
      setTypeColis(
        packageTypesData.find(
          (item) => item._id === selectedItem.typeColis._id,
        ),
      );
      setTransportType(
        transportTypesData.find(
          (item) => item._id === selectedItem.transportType._id,
        ),
      );
      setUnit(
        measureUnitsData.find((item) => item._id === selectedItem.unit._id),
      );
  ...
  }, [price, typeColis, transportType, unit, description, quantity]);
            ...
            <div className="w-[1104px] bg-white rounded-[12px]">
              ...
              <div className="mt-4 flex flex-col items-start gap-[16px] top-[94px] left-[32px]">
                <div className="flex w-[1040px] items-start gap-[12px] flex-[0_0_auto]">
                  {renderInputField(ADD_PRICING_INPUTS[0], price, (e) =>
                ...
                <div className="flex w-[1040px] items-start gap-[12px] relative flex-[0_0_auto]">
                  <div className="flex flex-row items-start gap-[8px] relative flex-1 grow">
                    {renderInputField(ADD_PRICING_INPUTS[4], description, (e) =>
                      setDescription(e.target.value),
                    )}
                    {renderInputField(ADD_PRICING_INPUTS[5], quantity, (e) =>
